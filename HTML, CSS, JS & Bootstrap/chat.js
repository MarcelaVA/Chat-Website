...
alertify.success('Hello, ' + msg['name'] + '!');
  ...
  createMessage(msg['text'], msg['sender']);
    ...
    createMyMessage(array[i]['text'], 
    new Date().getTime(), '');
    
    } else {
    createMessage(array[i]['text'], array[i]['sender']);
    }
    }
    }
};
...
function createMyMessage(text, label, disabled) {
  let messageContainerStr = '<li class = "message own ' +
disabled + '" id = "id' + label + '"> </li>';
let messageContainer = $(messageContainerStr);


let messageTextElementStr = '<p class = "message-text">' +
text + '</p>';
...
$(document).ready(function() {
  ...
  function createMessage(text, name) {
    let avatarStr = '<div class="message-avatar"><p>' + 
  name.charAt(0) + '</p></div>'
